...
import yatospace.user.session.control.MongoSessionController;
 ...
 * @version 1.0
 */
public class MongoSessionsTest {
	private static SessionDocumentConfigEngine configEngine = SessionDocumentConfigCenter.cofigurationsEngine;
	private static SessionDocumentDatabaseConfig config = configEngine.getDatabaseConfigurations();
	private static MongoSessionController sessionController = new MongoSessionController(config.getHost(), config.getPort(), config.getDatabase(), config.getCollection());
	...
	public static void login() {
		...
		if(exist!=null) System.out.println("Sesija vec postoji."); 
			...
			sessionController.getMemoryForSession().login(username, session);
